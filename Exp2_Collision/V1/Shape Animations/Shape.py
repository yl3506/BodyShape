...
class Shape(object):
    ...
    def __init__(self,coordinates,displayPara,initialY,angle,spatialGap,proper,code,flip,center):
        ...
        self.color.hsla=(360*random.random(),70,70,1)
    ...
    def adjustPosition(self):
        ...
        if self.property=="patient":
            ...
            for i in range(len(self.coordinates)):
                self.coordinates[i]=(movingLengthX+self.coordinates[i][0]-50,
                                     movingLengthY+self.coordinates[i][1]-80)
        else:
            movingLengthX,movingLengthY=self.movingLength()
            for i in range(len(self.coordinates)):
                self.coordinates[i]=(self.coordinates[i][0]+90,movingLengthY+
                                     self.coordinates[i][1]+20)#+20 if not square # -20 if square and aagent
...
def testing():
    ...
    eConcave=Shape([[34,33],[105,25],[98,82],[116,47],[91,122],[63,42],[44,109],[72,96],[47,123],[35,119]],
